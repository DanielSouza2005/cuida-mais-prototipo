...
public class ServiceRequestService {
  ...
  public ServiceRequestFormDataResponse formData(UUID responsibleId, UUID caregiverId) {
    ...
    CaregiverProfile caregiver = caregiverId == null ? null : requireCaregiver(caregiverId);
    ...
    AddressFields ca = caregiver == null ? null : caregiver.getEnderecoAtendimento();
    return new ServiceRequestFormDataResponse(
      caregiver == null ? null : new ServiceRequestFormDataResponse.Caregiver(caregiver.getId(), caregiver.getUser().getFullName(), caregiver.getUser().getProfilePhotoUrl(), ca == null ? null : ca.getCidade(), ca == null ? null : ca.getBairro(), ca == null ? null : ca.getEstado(), caregiver.getTempoExperiencia(), new LinkedHashSet<>(caregiver.getServicosOferecidos())),
  ...
  public ServiceRequestResponse create(UUID responsibleId, ServiceRequestCreateRequest request) {
    User responsible = requireResponsible(responsibleId);
    CaregiverProfile caregiver = request.caregiverId() == null ? null : requireCaregiver(request.caregiverId());
    ...
    history.record(StatusHistoryEntityType.SERVICE_REQUEST, saved.getId(), null, saved.getStatus().name(), responsible, null);
    if (caregiver != null) notifications.create(caregiver.getUser(),NotificationType.SERVICE_REQUEST_CREATED,"Nova solicitação de serviço","Você recebeu uma nova solicitação de "+responsible.getFullName()+".",saved.getId());
    else notifications.create(responsible, NotificationType.SERVICE_PUBLICATION_CREATED, "Serviço publicado", "Sua oportunidade de serviço foi publicada e já pode receber interessados.", RelatedEntityType.SERVICE_REQUEST, saved.getId());
  ...
  public ResponsibleServiceRequestResponse responsibleDetails(UUID responsibleId, UUID id) {
    ...
    if (entity.getCaregiverUser() == null) {
      throw new BusinessException("Esta publicação ainda não possui um cuidador interessado.", HttpStatus.CONFLICT);
    }
    ...
    return new ResponsibleServiceRequestResponse(
      entity.getId(), entity.getStatus(), entity.getInitiatedBy(),
  ...
  @Transactional
  public ServiceRequestResponse cancel(UUID responsibleId, UUID id) { User owner=requireResponsible(responsibleId); ServiceRequest entity=repository.findByIdAndResponsibleUser(id, owner).orElseThrow(() -> new BusinessException("Solicitação não encontrada.", HttpStatus.NOT_FOUND)); expireIfNeeded(entity); ServiceRequestStatus previous=entity.getStatus(); if(previous!=ServiceRequestStatus.PENDENTE&&previous!=ServiceRequestStatus.ABERTA) throw new BusinessException("Apenas solicitações pendentes ou serviços abertos podem ser cancelados."); entity.setStatus(ServiceRequestStatus.CANCELADA); entity.setCanceledAt(Instant.now()); history.record(StatusHistoryEntityType.SERVICE_REQUEST, entity.getId(), previous.name(), ServiceRequestStatus.CANCELADA.name(), owner, entity.getCancellationReason()); if(entity.getCaregiverUser()!=null) notifications.create(entity.getCaregiverUser(),NotificationType.SERVICE_REQUEST_CANCELED,"Solicitação cancelada","Uma solicitação de serviço foi cancelada pelo responsável.",entity.getId()); else notifications.create(owner, NotificationType.SERVICE_PUBLICATION_CANCELED, "Serviço publicado cancelado", "Sua publicação de serviço foi cancelada.", RelatedEntityType.SERVICE_REQUEST, entity.getId()); return toResponse(entity); }
  ...
  void expireIfNeeded(ServiceRequest e) { if((e.getStatus()==ServiceRequestStatus.PENDENTE||e.getStatus()==ServiceRequestStatus.ABERTA) && e.getExpiresAt().isBefore(Instant.now())) { ServiceRequestStatus previous=e.getStatus(); e.setStatus(ServiceRequestStatus.EXPIRADA); history.record(StatusHistoryEntityType.SERVICE_REQUEST, e.getId(), previous.name(), ServiceRequestStatus.EXPIRADA.name(), e.getResponsibleUser(), null); if(previous==ServiceRequestStatus.ABERTA) notifications.create(e.getResponsibleUser(), NotificationType.SERVICE_PUBLICATION_EXPIRED, "Serviço publicado expirado", "O prazo da sua publicação de serviço terminou.", RelatedEntityType.SERVICE_REQUEST, e.getId()); } }
  ...
  private CaregiverProfile requireCaregiver(UUID id) { return caregiverRepository.findById(id).filter(p -> (p.getUser().getUserType()==UserType.CUIDADOR || p.getUser().getUserType()==UserType.CAREGIVER) && p.getUser().isActive() && p.isApproved()).orElseThrow(() -> new BusinessException("Cuidador não encontrado.", HttpStatus.NOT_FOUND)); }
