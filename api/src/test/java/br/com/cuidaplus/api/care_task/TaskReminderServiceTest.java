...
class TaskReminderServiceTest {
  ...
  void overdueNotificationKeepsTheExistingOccurrenceAndCalculatedStatus() {
    ...
    when(occurrence.getStatus()).thenReturn(TaskOccurrenceStatus.PENDENTE);when(occurrence.getScheduledTime()).thenReturn(LocalTime.of(8,0));when(occurrence.getAssistedPerson()).thenReturn(assisted);
