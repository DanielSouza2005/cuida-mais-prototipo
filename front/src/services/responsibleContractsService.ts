...
type ApiListItem = {
  ...
  rejectionReason?: string | null; cancellationReason?: string | null; closureReason?: string | null;
};
...
function listItem(item: ApiListItem): ContractHistoryItem {
  return {
    ...
    participant: { id: '', name: item.participantName, profilePhotoUrl: item.participantPhotoUrl ?? undefined, locationSummary: '' },
    ...
    activities: [], needsDescription: '', rejectionReason: item.rejectionReason ?? undefined, cancellationReason: item.cancellationReason ?? undefined,
    closureReason: item.closureReason ?? undefined, createdAt: item.updatedAt, updatedAt: item.updatedAt, statusHistory: [],
  };
}
...
function detailsItem(item: ApiDetails): ContractHistoryItem {
  return {
    ...
    statusGroup: item.status === 'PENDENTE' ? 'PENDENTES' : item.status === 'ACEITA' || item.status === 'AGENDADA' ? 'AGENDADAS' : item.status === 'ATIVA' ? 'ATIVAS' : item.status === 'FINALIZADA' ? 'ENCERRADAS' : item.status === 'REJEITADA' ? 'REJEITADAS' : item.status === 'EXPIRADA' ? 'EXPIRADAS' : 'CANCELADAS',
...
export async function getResponsibleContracts(params: ContractsHistoryQuery): Promise<ContractsHistoryPage> {
  ...
  const response = await apiRequest<ApiPage>(`/api/responsible/contracts?${query.toString()}`);
...
export async function getResponsibleContractDetails(itemType: ContractHistoryItemType, id: string) {
  const response = await apiRequest<ApiDetails>(`/api/responsible/contracts/${itemType}/${id}`);
