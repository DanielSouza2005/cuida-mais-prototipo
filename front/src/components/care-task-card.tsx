...
export function TaskOccurrenceCard({ occurrence, onPress }: { occurrence: TaskOccurrence; onPress: () => void }) {
  return <Pressable accessibilityRole="button" onPress={onPress} style={({ pressed }) => [styles.card, pressed && styles.pressed]}>
    <View style={styles.top}><TaskStatusBadge status={occurrence.status} /><TaskPriorityBadge priority={occurrence.priority} /></View>
    <Text style={styles.time}>{formatScheduleTime(occurrence.scheduledTime)}</Text><Text style={styles.title}>{occurrence.title}</Text>
...
const styles = StyleSheet.create({
  card: { gap: spacing.sm, padding: spacing.lg, borderRadius: radii.xl, borderWidth: 1, borderColor: colors.border, backgroundColor: colors.card, ...shadows.card },
  ...
  lineText: { flex: 1, fontFamily: fontFamily.medium, fontSize: 11, color: colors.mutedForeground }, description: { fontFamily: fontFamily.regular, fontSize: 12, lineHeight: 18, color: colors.secondaryForeground },
});
