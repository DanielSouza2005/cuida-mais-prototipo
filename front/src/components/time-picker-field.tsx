...
export function TimePickerField({ label, value, onChange, required, optional, error, disabled, placeholder = 'Selecionar horário' }: Props) {
  ...
  function adjust(minutes: number) { setTemporary((current) => new Date(current.getTime() + minutes * 60_000)); }

  return <View style={styles.wrapper}>
    <Text style={styles.label}>{label}{optional ? ' (opcional)' : null}{required ? <Text style={styles.required}> *</Text> : null}</Text>
    <Pressable accessibilityRole="button" disabled={disabled} onPress={open} style={({ pressed }) => [styles.field, error && styles.fieldError, disabled && styles.disabled, pressed && !disabled && styles.pressed]}>
      <Clock color={colors.mutedForeground} size={19} /><Text style={[styles.value, !value && styles.placeholder]}>{value || placeholder}</Text>
    </Pressable>
    ...
    <Modal transparent animationType="fade" visible={visible && Platform.OS !== 'android'} onRequestClose={() => setVisible(false)}>
      <View style={styles.backdrop}><View style={styles.modalCard}><Text style={styles.modalTitle}>Selecionar horário</Text>
        {Platform.OS === 'ios' ? <DateTimePicker value={temporary} mode="time" is24Hour display="spinner" onChange={handleNativeChange} /> : <View style={styles.webPicker}><Pressable onPress={() => adjust(-30)} style={styles.adjust}><Text style={styles.adjustText}>− 30 min</Text></Pressable><Text style={styles.webTime}>{formatTime(temporary)}</Text><Pressable onPress={() => adjust(30)} style={styles.adjust}><Text style={styles.adjustText}>+ 30 min</Text></Pressable></View>}
...
const styles = StyleSheet.create({
  ...
  backdrop: { flex: 1, justifyContent: 'center', padding: spacing.xl, backgroundColor: 'rgba(27,45,64,0.34)' }, modalCard: { gap: spacing.lg, padding: spacing.lg, borderRadius: radii.xl, backgroundColor: colors.card, ...shadows.soft }, modalTitle: { fontFamily: fontFamily.bold, fontSize: 17, color: colors.foreground, textAlign: 'center' }, actions: { flexDirection: 'row', gap: spacing.sm }, action: { flex: 1, minHeight: 46, alignItems: 'center', justifyContent: 'center', borderRadius: radii.lg, backgroundColor: colors.secondary }, confirm: { backgroundColor: colors.primary }, cancel: { fontFamily: fontFamily.semiBold, color: colors.primary }, confirmText: { fontFamily: fontFamily.semiBold, color: colors.primaryForeground }, webPicker: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', gap: spacing.sm }, adjust: { padding: spacing.md, borderRadius: radii.lg, backgroundColor: colors.secondary }, adjustText: { fontFamily: fontFamily.semiBold, fontSize: 12, color: colors.primary }, webTime: { fontFamily: fontFamily.extraBold, fontSize: 24, color: colors.foreground },
});
