...
import { activityLabels, hiringLabels, weekdayLabels } from '@/utils/serviceRequestLabels';
...
export default function RequestServiceReviewScreen() {
  ...
  if (!draft?.hiringType) return <ScreenContainer contentStyle={styles.center}><Text style={styles.text}>Preencha a solicitação antes de revisar.</Text><PrimaryButton label="Voltar ao formulário" onPress={() => router.replace('/request-service' as Href)} /></ScreenContainer>;
  ...
  async function confirm() {
    ...
    const payload: ServiceRequestPayload = {
      caregiverId: draft.caregiver.id, assistedPersonId: draft.assistedPerson.id, careAddressId: draft.address.id, hiringType: draft.hiringType,
      ...
      needsDescription: draft.needsDescription, activities: draft.activities, activityOther: draft.otherActivity || null, additionalNotes: draft.additionalNotes || null, negotiationNotes: draft.negotiation || null,
    };
  ...
  return (
    <ScreenContainer contentStyle={styles.content}>
      ...
      <Card title="Necessidades e atividades"><Text style={styles.text}>{draft.needsDescription}</Text><Row label="Atividades" value={draft.activities.map((activity) => activity === 'OUTRO' ? draft.otherActivity : activityLabels[activity] ?? activity).join(', ')} />{draft.additionalNotes ? <Row label="Observações" value={draft.additionalNotes} /> : null}{draft.negotiation ? <Row label="Negociação" value={draft.negotiation} /> : null}</Card>
