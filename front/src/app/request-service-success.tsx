...
export default function RequestServiceSuccessScreen() {
  ...
  return (
    <ScreenContainer contentStyle={styles.content}>
      ...
      <View style={styles.summary}><Summary label="Cuidador" value={request.caregiverName} /><Summary label="Pessoa assistida" value={request.assistedPersonName} /><Summary label="Tipo de contratação" value={hiringLabels[request.hiringType]} /><Summary label="Data de início" value={start ?? 'Não informada'} />{request.scheduleDays[0] ? <Summary label="Horário" value={`${request.scheduleDays[0].startTime.slice(0, 5)} às ${request.scheduleDays[0].endTime.slice(0, 5)}`} /> : null}<Summary label="Expira em" value={new Date(request.expiresAt).toLocaleDateString('pt-BR')} /></View>
